// Tetromino Class
  ...
  $.extend(Tetromino.prototype, {
    ...
    move: function(delta){
      ...
      for (var i=0; i < cells.length; i++) {
        if (!board.isLegit(this.calcPosition(cells[i], delta))) return;
      }
    ...
    isFinished: function(){
      var cells = this.cells;
      for (var i=0; i < cells.length; i++) {
        if (this.calcPosition(cells[i], [0,0])[1] == this.board.height - 1 || !board.isLegit(this.calcPosition(cells[i], [0,1]))){
          return true;
        }
      }
      return false;
